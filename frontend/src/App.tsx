...
export default function App() {
  ...
  const fetchGatewayBalance = async () => {
    ...
    try {
      const res = await apiClient.get('/messaging/admin/routes/balance');
  ...
  useEffect(() => {
    ...
    const mappedChats = Object.keys(threadedConversations).map(msisdn => {
      ...
      return {
        ...
        time: parseSafeDate(lastTimestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    ...
    mappedChats.sort((a, b) => {
      return parseSafeDate(b.lastTimestamp).getTime() - parseSafeDate(a.lastTimestamp).getTime();
    });
                ...
                {user.role_tier === 'CLIENT' && clientAccount && (
                  <>
                    ...
                    <div className="bg-slate-900/90 border border-emerald-500/40 px-3.5 md:px-5 py-1.5 rounded-full flex items-center gap-1.5 md:gap-2 text-emerald-400 font-mono text-xs md:text-sm shadow-[0_0_20px_rgba(16,185,129,0.12)]">
                      ...
                      <span className="bg-cyan-500/20 text-cyan-300 text-[10px] md:text-xs font-bold px-2 py-0.5 rounded-full border border-cyan-500/30 font-sans hidden sm:inline-block">
                        ⚡ {Math.floor(Number(clientAccount.wallet_balance) / 1.5).toLocaleString()} SMS
                      </span>
                    </div>
                    <button 
                                ...
                                clients.filter((c: any) => 
                                  ...
                                  <tr key={client.id} className="hover:bg-slate-900/40 transition-colors">
                                    ...
                                    <td className="py-3.5 px-4 font-mono font-black text-emerald-400 text-sm">
                                      <div>Ksh {(parseFloat(client.wallet_balance) || 0).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</div>
                                      <div className="text-[10px] text-cyan-300 font-bold font-sans">
                                        ⚡ {Math.floor((parseFloat(client.wallet_balance) || 0) / 1.5).toLocaleString()} SMS Units
                                      </div>
                                    </td>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div className="glass-panel p-6 rounded-3xl border border-indigo-500/20 glow-card flex items-center justify-between shadow-xl transition-all duration-300 hover:scale-[1.02] hover:border-emerald-500/40">
                          <div>
                            ...
                            <p className="text-[11px] text-cyan-300 mt-1.5 font-bold flex items-center gap-1 font-sans">
                              ⚡ {Math.floor(Number(clientAccount.wallet_balance) / 1.5).toLocaleString()} SMS Units
                              <span className="text-slate-400 font-normal">(@ Ksh 1.50/SMS)</span>
                            </p>
                          </div>
                          <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-emerald-500/20 to-teal-500/20 border border-emerald-500/30 flex items-center justify-center text-emerald-400 shadow-lg shadow-emerald-500/10">
                            <Wallet className="w-7 h-7" />
                    ...
                    <div className="h-[78vh] flex bg-slate-900/60 backdrop-blur-2xl border border-indigo-500/20 rounded-3xl overflow-hidden animate-fade-in-up shadow-[0_16px_50px_rgba(0,0,0,0.7),0_0_35px_rgba(99,102,241,0.1)]">
                      ...
                      <div className={`w-full md:w-80 border-r border-slate-800/80 flex flex-col bg-slate-950/60 ${selectedChatId ? 'hidden md:flex' : 'flex'}`}>
                        ...
                        <div className="flex-1 overflow-y-auto overflow-x-hidden">
                          {(() => {
                            const filteredChats = inboxChats.filter(chat => inboxTab === 'archived' ? chat.hasArchived : chat.hasActive);
                            
                            // Group sidebar chats into date sections
                            const chatsByDate: { dateHeader: string; chats: any[] }[] = [];
                            filteredChats.forEach(chat => {
                              const header = chat.dateHeader || 'Today';
                              const lastGroup = chatsByDate[chatsByDate.length - 1];
                              if (lastGroup && lastGroup.dateHeader === header) {
                                lastGroup.chats.push(chat);
                              } else {
                                chatsByDate.push({ dateHeader: header, chats: [chat] });
                              }
                            });
